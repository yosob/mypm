...
> **2026-08-29 更新**：工具集从 9 个扩到 14 个，看板新增属性（排期/优先级/项目状态与截止日/任务级资料/自定义字段）已全部双向接入 AI（第四节工具总表、第七节落地清单）。原则：**看板能做的 AI 都能做，AI 做的看板都能看**；唯一非对称是删除——仅看板可删，AI 永不删（防误删）。
>
> **2026-08-31 更新**：工具数核定为 15（此前文档口径漂移为 11/14）；同日按工具设计方法论（Anthropic《Writing effective tools for agents》等，见 DECISIONS #40）**合并为 10 个**——消除重叠、每个实体一条写路径；`create_task`/`update_task` 新增 `parent_task_id` 参数接入子任务（DECISIONS #39）。
...
你(Lark): "确认"
  │
  ▼ 同一 Agent 实例（有上下文，记得刚才的清单）→ AI 调 resolve_updates({action:"apply", update_id})
...
**关键点**：确认状态不在 AI 里，在 `pending_updates` 表里。AI 只是"念出"清单和"执行"resolve_updates——就算 AI 忘了上下文，update_id 还在库里，你说"应用刚才的更新"它查表也能完成。这是把状态外置的 AI native 设计，防幻觉。
...
### 工具总表（v3，共 10 个，2026-08-31 按 DECISIONS #40 合并）

| 工具 | 读/写 | 覆盖的看板功能 |
|---|---|---|
| `list_projects` | 读 | 不传参数=项目总览（状态/目标/截止日+任务）；传 project=单项目完整详情（+资料+历史时间线） |
| `list_tasks` | 读 | 任务查询（项目/截止窗口过滤；id/排期/状态/优先级/逾期标记；子任务带 └ 前缀） |
| `get_task` ★ | 读 | 单任务全量：排期/状态/优先级/内容/任务级资料(群/链接)/自定义字段/父子任务 |
| `propose_updates` | 写 | 纪要→拟更新（提取排期/优先级/里程碑/资源/摘要，两段式确认的入口） |
| `resolve_updates` | 写 | 处理拟更新：action=apply 确认入库 / discard 丢弃（幂等） |
| `create_task` | 写 | 新建（项目/标题/开始日/截止日/优先级/里程碑/备注）；传 parent_task_id 建子任务（归属自动取父项目） |
...
3. 只有用户明确同意后才调 resolve_updates（action=apply，用清单编号）。用户拒绝或要求重新
   调整时：调 resolve_updates（action=discard）丢弃，再按修改意见用 create_task 等逐条执行并汇报。
...
10. 设置附加属性（负责人、合同号等自定义字段）：调 update_task 传 custom 参数（字段名→值）。
11. 项目目标/状态/截止日修改：调 update_project。
12. 回复简洁中文，列表优先，不寒暄。意图不明时列选项让用户选，不自作主张写库。
13. 任务拆解：用户要"给任务X加个子任务/把X拆解一下"时，用 create_task 传 parent_task_id
    （归属自动取父任务所在项目，最多一层，子任务不能再挂子任务）；父任务完成时未完成的
    子任务自动完成；解除关联用 update_task 把 parent_task_id 置 0。
```
...
[10 个工具的 JSON schema（~1100 token/次，随每轮重发）]
[当前新消息]
```

**管理策略**（2026-08-29 升级为"滚动摘要"）：
1. 单会话最多保留 **SESSION_MAX=200** 条原始消息（config.json app 节可调）
2. 超过即触发压缩：最早的 (200-50) 条按**安全边界**切出（不落在工具调用/结果中间），
   连同旧摘要交给 GLM 合并成 ≤500 字**滚动摘要**；保留最近 SESSION_KEEP=50 条原始消息
3. 摘要在下轮注入为开头一条"背景消息"，AI 获得长程记忆；压缩失败则保留旧摘要不丢数据
4. `pending_updates`、任务库本身仍是**外部记忆**：AI 随时可查，不依赖会话记忆——会话丢了数据不丢
