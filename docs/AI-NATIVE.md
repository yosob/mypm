...
>
> **2026-08-31 更新**：工具数核定为 **15 个**（此前文档口径漂移为 11/14）；`create_task`/`update_task` 新增 `parent_task_id` 参数接入子任务（DECISIONS #39）。
...
| `update_task` | 写 | 改期(起止)/状态/完成/改名/优先级/父子关联（parent_task_id 置 0 解除）；父完成自动级联子完成 |
...
| `set_timer` ★ | 写 | 定时提醒：一次性(run_at)或周期(cron)，到点机器人私聊卡片 |
| `list_timers` / `cancel_timer` ★ | 读/写 | 查询/取消定时提醒 |
...
[15 个工具的 JSON schema（~1200 token/次，随每轮重发）]
[当前新消息]
```
