...
## 决议 #39（2026-08-31）：子任务（深度一层 / 同项目 / 级联完成）

- **深度最多一层**：子任务不能再挂子任务。避免无限层级的展开/折叠/成环复杂性，
  覆盖 checklist 式拆解主场景；dhtmlxGantt 树天然支持"项目→任务→子任务"三层
- **同项目强制**：甘特树 parent 必须同树内、列表按项目分组——跨项目父子会撕裂展示，
  技术约束直接定为校验规则（db.validateParent：存在/同项目/一层/不自引用）
- **级联完成**：父任务完成时未完成子任务自动完成（Todoist/飞书同款）；子任务完成
  不影响父任务。级联放在 db.updateTask 内，AI 与看板走同一路径天然一致
- 数据库零迁移：parent_id 列自建库即在；API 层补校验（顺带解决优化清单 P13 的
  parent 部分）；工具层仅 create_task/update_task 各加 parent_task_id 可选参数，
  get_task 原已展示父子；prompt 加规则 13
...
## 决议 #41（2026-08-31）：让 AI 感知系统调度（三层注入，结构上不可删）

- **背景**：dad5033 部署版暴露两个问题——①B4 bug（先标记后发送，发送失败当日提醒
  静默丢失，用户"今天到期的任务没提醒到"）；②AI 被问"怎么没提醒我"时谎称"没有
  主动提醒功能"（每日提醒 cron 不经 agent，prompt 零感知）
- **方案（用户提出方向：cron 让 AI 可感知、但不可删）**，三层：
  ① 感知层：systemPrompt 每轮注入"系统自动调度"块（cron/窗口参数来自 config）+
    上次运行遥测——AI 能用真实状态回答"怎么没提醒我"
  ② 遥测层：runCheck 每次运行把 {时间,成功,条数/原因} 写 settings kv（last_check），
    零迁移；发送失败时 AI 可如实说"上次失败、将自动重试"
  ③ 一致性层：timer(action=list) 末尾附"[系统] 每日项目提醒…（内置，不可取消）"，
    用户问"我有哪些提醒"时两类同屏，与 prompt 口径一致
- **不可删除是结构保证**：系统 cron 由 config.app.remindCron 驱动，不在 timers 表、
  无数字 id，timer(action=cancel) 物理上碰不到；改节奏只能改配置文件（人类操作）
- 否决纯"堵嘴"式修法（prompt 只写"不要说没有功能"）：没有状态可讲的规矩是台词，
  有遥测的回答才是感知（同 #36"动态廉价信息进 prompt"原则）
